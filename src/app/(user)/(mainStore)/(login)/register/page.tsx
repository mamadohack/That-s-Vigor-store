...
const Page = ({ }) => {
  ...
  const signUPSCHEMA = z.object({
    email_Add: z.string().min(1, { message: "Email Required" }).email({ message: 'Email is not Valid !' }),
  ...
  const onSubmit: SubmitHandler<signup> = (data) => {
    const loginCred = { email: data.email_Add, password: data.Password, username: data.username };
